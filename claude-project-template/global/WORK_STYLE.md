...
| **Hands-on testing goes to the tester agent** | After a pause-point build, spawn the global `tester` subagent (Sonnet) to walk the click list on the real device, browser or CLI and report PASS/FAIL with literal evidence; also for reproducing a bug and collecting logs. It never edits the repo, stops at credential gates, and its report is evidence, not a verdict: the user still closes the pause. | Tester agent |
