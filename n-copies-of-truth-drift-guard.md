...
2. **Asserts set-equality (or a defined superset relationship)** between each pair, and prints a **diff table** on mismatch — which layer has the field, which doesn't — so the failure is immediately actionable, not a generic "schemas don't match."
   - **A superset relationship is satisfiable by ABSENCE, which makes it blind exactly where a new table appears.** This is the sharp edge on the recommendation above, and it cost a total sync outage to find. Treating the server's write-allowlist as a permitted *subset* of the synced columns is correct — a table can legitimately omit `created_at` and let the server default it — but "subset" is trivially true when the new table is missing from the allowlist altogether. So the check stays **green** while the client writes a column the server will reject, and the guard built specifically to catch cross-layer drift structurally cannot express the mismatch. The direction it never checks is the one that matters: **assert the client's INSERT column list against the server's accept-list**, table by table, and fail when the client writes something the server will not take. A superset assertion answers "is anything missing here?"; it never answers "is anything extra over there?" — and at a write boundary, extra is the one that takes production down. See [strict-schema-at-a-sync-boundary.md](./strict-schema-at-a-sync-boundary.md) for what the resulting outage looks like from the symptom end.
...
- [`strict-schema-at-a-sync-boundary.md`](./strict-schema-at-a-sync-boundary.md) — the outage this guard's superset blind spot lets through: one unknown column rejects a whole batch, the ordered queue stalls, and every table stops syncing.
