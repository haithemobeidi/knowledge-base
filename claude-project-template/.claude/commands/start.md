...
2. Read the **open `[ ]` lines only** of `docs/SESSION_LEDGER.md` — the append-and-strike ledger of open session-scoped items (queued tests, gates, riders). Grep for `- [ ]` rather than reading the whole file: closed `[x]`/`[-]` lines keep their full pre-closure text struck through, so they run hundreds of words each and are history, not context. Count the open items and note any that bear on the NEXT ACTION (e.g. an open pre-release gate when the next action is a release). Read a closed line only if something this session explicitly points at it.
